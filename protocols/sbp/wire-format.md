---
url: /protocols/sbp/wire-format.md
---
# SBP Wire Format (v1)
...
* Requests/responses/events all reuse `Message` frames (`t=1`). RPC correlation is defined at the envelope layer (see RPC envelope spec): `cid` inside `MessageFrame.data` is used for request/response matching. `frameId` remains sender-local unique and is *not* reused by receivers. See ADR-010.
